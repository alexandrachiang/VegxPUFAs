...
manhattancex <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
    ...
    if (!is.null(highlight)) {
        ...
        with(d.highlight, points(pos, logp, col=highlightcol, pch=20, cex = highlightcex, ...)) 
    }
    ...
    if (!is.null(annotatePval)) {
        ...
        if (annotateTop == FALSE) {
		    ...
		    if (!is.null(highlight)) {
			    ...
			    significantSNPs <- d[d$SNP %in% highlight, ]
			    suggestedSNPs <- topSNPs[!topSNPs$SNP %in% highlight, ]
